...
namespace UndertaleModToolAvalonia
{
    ...
    public partial class UndertaleCodeEditor : DataUserControl
    {
        ...
        private async void UserControl_DataContextChanged(object sender, AvaloniaPropertyChangedEventArgs e)
        {
            UndertaleCode code = this.DataContext as UndertaleCode;
            if (code == null)
                return;
            ...
            Task t = Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (MessageBox.Show("Unable to execute GraphViz: " + e.Message + "\nMake sure you have downloaded it and set the path in settings.\nDo you want to open the download page now?", "Graph generation failed", MessageBoxButton.YesNo, MessageBoxImage.Error) == ButtonResult.Yes)
