...
namespace UndertaleModToolAvalonia
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IScriptInterface
    {
        ...
        public void NukeProfileGML(string codeName)
        {
            ...
            string path = Path.Combine(ProfilesFolder, UndertaleSequence.AudioKeyframes.Data.ToolInfo.CurrentMD5, "Temp", codeName + ".gml");
            if (File.Exists(path))
            {
                File.Delete(path);
            }
        ...
        {
            foreach (UndertaleCode code in UndertaleSequence.AudioKeyframes.Data.Code)
            {
                string path = Path.Combine(ProfilesFolder, UndertaleSequence.AudioKeyframes.Data.ToolInfo.CurrentMD5, "Temp", code.Name.Content + ".gml");
                if (File.Exists(path))
                {
        ...
        {
            UndertaleCode code = UndertaleSequence.AudioKeyframes.Data.Code.ByName(codeName);
            if (code is null)
        ...
        public void ReplaceTextInGML(UndertaleCode code, string keyword, string replacement, bool case_sensitive = false, bool isRegex = false, GlobalDecompileContext context = null)
        {
            ...
            GlobalDecompileContext DECOMPILE_CONTEXT = context is null ? new(UndertaleSequence.AudioKeyframes.Data, false) : context;

            if (UndertaleSequence.AudioKeyframes.Data.ToolInfo.ProfileMode == false || UndertaleSequence.AudioKeyframes.Data.GMS2_3)
            {
                try
                {
                    passBack = GetPassBack((code != null ? Decompiler.Decompile(code, DECOMPILE_CONTEXT ) : ""), keyword, replacement, case_sensitive, isRegex);
                    code.ReplaceGML(passBack, UndertaleSequence.AudioKeyframes.Data);
                }
                catch (Exception exc)
                {
            ...
            else if (UndertaleSequence.AudioKeyframes.Data.ToolInfo.ProfileMode && !UndertaleSequence.AudioKeyframes.Data.GMS2_3)
            {
                try
                {
                    string path = Path.Combine(ProfilesFolder, UndertaleSequence.AudioKeyframes.Data.ToolInfo.CurrentMD5, "Temp", codeName + ".gml");
                    ...
                    else
                    {
                        ...
                        {
                            if (context is null)
                                passBack = GetPassBack((code != null ? Decompiler.Decompile(code, new GlobalDecompileContext(UndertaleSequence.AudioKeyframes.Data, false)) : ""), keyword, replacement, case_sensitive, isRegex);
                            else
        ...
        void ImportCodeFromFile(string file, bool IsGML = true, bool doParse = true, bool destroyASM = true, bool CheckDecompiler = false, bool throwOnError = false)
        {
            try
            {
                if (!Path.GetFileName(file).ToLower().EndsWith(IsGML ? ".gml" : ".asm"))
                    return;
                if (Path.GetFileName(file).ToLower().EndsWith("cleanup_0" + (IsGML ? ".gml" : ".asm")) && (UndertaleSequence.AudioKeyframes.Data.GeneralInfo.Major < 2))
                    return;
                if (Path.GetFileName(file).ToLower().EndsWith("precreate_0" + (IsGML ? ".gml" : ".asm")) && (UndertaleSequence.AudioKeyframes.Data.GeneralInfo.Major < 2))
                    return;
        ...
        void ImportCode(string codeName, string gmlCode, bool IsGML = true, bool doParse = true, bool destroyASM = true, bool CheckDecompiler = false, bool throwOnError = false)
        {
            ...
            if (code is null)
            {
                ...
                code.Name = UndertaleSequence.AudioKeyframes.Data.Strings.MakeString(codeName);
                UndertaleSequence.AudioKeyframes.Data.Code.Add(code);
            }
            if (Data?.GeneralInfo.BytecodeVersion > 14 && UndertaleSequence.AudioKeyframes.Data.CodeLocals.ByName(codeName) == null)
            {
                UndertaleCodeLocals locals = new UndertaleCodeLocals();
                locals.Name = code.Name;

                UndertaleCodeLocals.LocalVar argsLocal = new UndertaleCodeLocals.LocalVar();
                argsLocal.Name = UndertaleSequence.AudioKeyframes.Data.Strings.MakeString("arguments");
                argsLocal.Index = 0;
                ...
                UndertaleSequence.AudioKeyframes.Data.CodeLocals.Add(locals);
            }
            if (doParse)
            {
                ...
                if (codeName.Substring(0, 10).Equals("gml_Script"))
                {
                    ...
                    if (UndertaleSequence.AudioKeyframes.Data.Scripts.ByName(codeName.Substring(11)) == null)
                    {
                        UndertaleScript scr = new UndertaleScript();
                        scr.Name = UndertaleSequence.AudioKeyframes.Data.Strings.MakeString(codeName.Substring(11));
                        scr.Code = code;
                        UndertaleSequence.AudioKeyframes.Data.Scripts.Add(scr);
                    }
                    else
                    {
                        UndertaleScript scr = UndertaleSequence.AudioKeyframes.Data.Scripts.ByName(codeName.Substring(11));
                        scr.Code = code;
                    }
                }
                else if (codeName.Substring(0, 16).Equals("gml_GlobalScript"))
                {
                    ...
                    foreach (UndertaleGlobalInit globalInit in UndertaleSequence.AudioKeyframes.Data.GlobalInitScripts)
                    {
                    ...
                    if (init_entry == null)
                    {
                        ...
                        NewInit.Code = code;
                        UndertaleSequence.AudioKeyframes.Data.GlobalInitScripts.Add(NewInit);
                    }
                    else
                    {
                ...
                else if (codeName.Substring(0, 10).Equals("gml_Object"))
                {
                    ...
                    try
                    {
                        ...
                        if (methodName == "Collision" && (methodNumber >= UndertaleSequence.AudioKeyframes.Data.GameObjects.Count || methodNumber < 0))
                        {
                            ...
                            if (doNewObj)
                            {
                                ...
                                gameObj.Name = UndertaleSequence.AudioKeyframes.Data.Strings.MakeString(SimpleTextInput("Enter object name", "Enter object name", "This is a single text line input box test.", false));
                                UndertaleSequence.AudioKeyframes.Data.GameObjects.Add(gameObj);
                            }
                            else
                            {
                    ...
                    catch
                    {
                        if (afterPrefix.LastIndexOf("_Collision_") != -1)
                        {
                            ...
                            if (UndertaleSequence.AudioKeyframes.Data.GMS2_3)
                            {
                                if (UndertaleSequence.AudioKeyframes.Data.GameObjects.ByName(methodNumberStr) != null)
                                {
                                    for (var i = 0; i < UndertaleSequence.AudioKeyframes.Data.GameObjects.Count; i++)
                                    {
                                        if (UndertaleSequence.AudioKeyframes.Data.GameObjects[i].Name.Content == methodNumberStr)
                                        {
                                            methodNumber = i;
                                            break;
                                        }
                                ...
                                else
                                {
                                    ...
                                    {
                                        UndertaleGameObject gameObj = new UndertaleGameObject();
                                        gameObj.Name = UndertaleSequence.AudioKeyframes.Data.Strings.MakeString(objName);
                                        UndertaleSequence.AudioKeyframes.Data.GameObjects.Add(gameObj);
                                    }
                                }
                                if (UndertaleSequence.AudioKeyframes.Data.GameObjects.ByName(methodNumberStr) != null)
                    ...
                    UndertaleGameObject obj = UndertaleSequence.AudioKeyframes.Data.GameObjects.ByName(objName);
                    if (obj == null)
                    {
                        bool doNewObj = ScriptQuestion("Object " + objName + " was not found.\nAdd new object called " + objName + "?");
                        if (doNewObj)
                    ...
                    if (!(SkipPortions))
                    {
                        obj = UndertaleSequence.AudioKeyframes.Data.GameObjects.ByName(objName);
        ...
        void SafeImport(string codeName, string gmlCode, bool IsGML, bool destroyASM = true, bool CheckDecompiler = false, bool throwOnError = false)
        {
            ...
            try
            {
                if (IsGML)
                {
                    code.ReplaceGML(gmlCode, UndertaleSequence.AudioKeyframes.Data);
                ...
                {
                    var instructions = Assembler.Assemble(gmlCode, UndertaleSequence.AudioKeyframes.Data);
            ...
            catch (Exception ex)
            {
                ...
                {
                    code.ReplaceGML("", UndertaleSequence.AudioKeyframes.Data);
                }
            }
        }
    }
